...
- **Provider name mismatch silently drops all cloud models**: `provider.ts:160` must use `opencode:` (not `cody:`) as the provider ID prefix. The `models.dev` API returns models keyed by `opencode`; the matching prefix at line 160 (`database[providerID]`) extracts them. A mismatch (`database[providerID]` → undefined → `continue`) silently skips every cloud model — no error, no warning, the API response is ignored.

## Branding

- **Brand env var tristate**: `CODY_X=1` + `CODY_PRO=0` must be set together. `CODY_PRO` is checked with truthy semantics throughout the codebase (not `=== "1"`), so without explicitly setting `CODY_PRO=0`, the cody-pro branding path would still match. Neither set → falls back to "cody" (bare clone).

## Server

- **Default port 4096** is hardcoded at `src/server/adapter.bun.ts:16`. Falls back to OS-assigned random if 4096 is busy. Override with `--port <N>`.
