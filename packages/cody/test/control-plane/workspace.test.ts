...
const originalWorkspacesFlag = Flag.CODY_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  CODY_AUTH_CONTENT: process.env.CODY_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.CODY_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.CODY_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      ...
      expect(JSON.parse(recorded.calls.create[0].env.CODY_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.CODY_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.CODY_EXPERIMENTAL_WORKSPACES).toBe("true")
