...
const auth = { username: "cody", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.CODY_SERVER_PASSWORD
  else process.env.CODY_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CODY_SERVER_USERNAME
  else process.env.CODY_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener(backend: "effect-httpapi" | "hono" = "effect-httpapi") {
  Flag.CODY_EXPERIMENTAL_HTTPAPI = backend === "effect-httpapi"
  Flag.CODY_SERVER_PASSWORD = auth.password
  Flag.CODY_SERVER_USERNAME = auth.username
  process.env.CODY_SERVER_PASSWORD = auth.password
  process.env.CODY_SERVER_USERNAME = auth.username
