 ...
 *
 * The script intentionally isolates `CODY_DB` before importing modules that touch
...
void (await import("@cody/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "cody", model: "test" })}`,
      headers: ctx.headers(),
    }))
