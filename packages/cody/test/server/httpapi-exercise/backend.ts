...
function app(modules: Runtime, backend: Backend, options: CallOptions) {
  ...
  Flag.CODY_EXPERIMENTAL_HTTPAPI = backend === "effect"
  Flag.CODY_SERVER_PASSWORD = password
  Flag.CODY_SERVER_USERNAME = username
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ CODY_SERVER_PASSWORD: password, CODY_SERVER_USERNAME: username }),
        ),
      ),
    ),
    { disableLogger: true },
