...
import { EffectFlock } from "@cody/core/util/effect-flock"
...
const managedConfigDir = process.env.CODY_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["CODY_CONSOLE_TOKEN"]
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["cody"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["CODY_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CODY_CONSOLE_TOKEN"]
    }
  }
})
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".cody", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("does not try to install dependencies in read-only CODY_CONFIG_DIR", async () => {
  ...
  process.env.CODY_CONFIG_DIR = tmp.extra
...
test("installs dependencies in writable CODY_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".cody")
...
// Note: preload.ts sets CODY_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.cody/plugin/demo.ts", "file:///project/.cody/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.cody/plugin/demo.ts"])
  })
...
describe("CODY_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CODY_DISABLE_PROJECT_CONFIG"]
    process.env["CODY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CODY_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CODY_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .cody/ directories when flag is set", async () => {
    const originalEnv = process.env["CODY_DISABLE_PROJECT_CONFIG"]
    process.env["CODY_DISABLE_PROJECT_CONFIG"] = "true"
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CODY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODY_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CODY_CONFIG_DIR"]
      process.env["CODY_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CODY_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CODY_CONFIG_DIR"]
      } else {
        process.env["CODY_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CODY_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CODY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODY_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CODY_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CODY_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from CODY_CONFIG_DIR, not project
...
describe("CODY_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CODY_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODY_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CODY_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://cody.dev/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CODY_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CODY_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CODY_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODY_CONFIG_CONTENT"]
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://cody.dev/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://cody.dev/config.json")
})
