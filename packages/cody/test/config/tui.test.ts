...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".cody")
...
test("project config takes precedence over CODY_TUI_CONFIG (matches CODY_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CODY_TUI_CONFIG = custom
    },
  })
...
test("CODY_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from CODY_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("loads .cody/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".cody"), { recursive: true })
      await Bun.write(path.join(dir, ".cody", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("silently skips malformed tui.json — load failures degrade to {}", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, ".cody", "tui.json"), JSON.stringify({ theme: "fallback" }))
    },
  })

  const config = await getTuiConfig(tmp.path)
  // Project tui.json is malformed → silently skipped (logs a warning)
  // .cody/tui.json (lower precedence in this path) still loads
...
test("silently skips non-ENOENT read failures (e.g. tui.json is a directory) — fallback layer still loads", async () => {
  ...
  const config = await getTuiConfig(tmp.path)
  // Did NOT crash; .cody/tui.json (lower precedence) still loads.
  expect(config.theme).toBe("fallback")
})
