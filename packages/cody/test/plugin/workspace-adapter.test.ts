...
const disableDefault = process.env.CODY_DISABLE_DEFAULT_PLUGINS
process.env.CODY_DISABLE_DEFAULT_PLUGINS = "1"

const { Flag } = await import("@cody/core/flag/flag")
...
const experimental = Flag.CODY_EXPERIMENTAL_WORKSPACES
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.CODY_DISABLE_DEFAULT_PLUGINS
  } else {
    process.env.CODY_DISABLE_DEFAULT_PLUGINS = disableDefault
  }

  Flag.CODY_EXPERIMENTAL_WORKSPACES = experimental
})
