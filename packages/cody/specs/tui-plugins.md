...
- Author package entrypoint is `@cody/plugin/tui`.
...
- Import types from `@cody/plugin/tui`.
- `@cody/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@cody/plugin/tui"
...
That is what makes local config-scoped plugins able to import `@cody/plugin/tui`.
...
- For generic config-to-bindings helpers, import `resolveBindingSections` from `@cody/plugin/tui`.
...
- Local plugins persist installed themes under the local `.cody/themes` area near the plugin config source.
...
- `--pure` / `CODY_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.cody/plugins/tui-smoke.tsx`
- Local vim plugin: `.cody/plugins/tui-vim.tsx`
- Local smoke config: `.cody/tui.json`
- Local smoke theme: `.cody/plugins/smoke-theme.json`
