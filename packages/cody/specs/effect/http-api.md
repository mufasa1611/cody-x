...
- `CODY_EXPERIMENTAL_HTTPAPI` selects the backend at server startup. Default is still `hono`.
...
- An Effect `HttpApi` OpenAPI surface exists (`OpenApi.fromApi(PublicApi)` in `cli/cmd/generate.ts --httpapi`, `CODY_SDK_OPENAPI=httpapi` in `packages/sdk/js/script/build.ts`) but is opt-in. The default SDK generation is still Hono.
...
3. Mount the new paths in `src/server/routes/instance/index.ts` only inside the `CODY_EXPERIMENTAL_HTTPAPI` block.
...
2. The route is mounted by default, not only behind `CODY_EXPERIMENTAL_HTTPAPI`.
...
14. [ ] Switch OpenAPI/SDK generation to Effect routes and compare SDK output. Effect path is implemented and opt-in via `--httpapi` / `CODY_SDK_OPENAPI=httpapi`. Close the schema-shape gaps in `public.ts` (branded `pattern`, per-property `description`, `Event.*` / `SyncEvent.*` naming, dedup collisions), then flip `packages/sdk/js/script/build.ts` default.
15. [ ] Flip `backend.ts` default from `hono` to `effect-httpapi`, keep `CODY_EXPERIMENTAL_HTTPAPI` (or its inverse) as a short fallback flag, then delete replaced Hono route files.

## Checklist

- [x] Add first `HttpApi` JSON route slices.
- [x] Bridge selected `HttpApi` routes behind `CODY_EXPERIMENTAL_HTTPAPI`. (Now backend-fork-at-startup rather than in-Hono path mounting.)
...
- [x] Implement Effect `HttpApi` OpenAPI generation behind `--httpapi` / `CODY_SDK_OPENAPI=httpapi`.
