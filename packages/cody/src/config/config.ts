...
import { InstallationLocal, InstallationVersion } from "@cody/core/installation/version"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://cody.dev/config.json"
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://cody.dev/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODY_CONFIG_CONTENT") return "local"
        ...
        if (Flag.CODY_CONFIG) {
          yield* merge(Flag.CODY_CONFIG, yield* loadFile(Flag.CODY_CONFIG))
          log.debug("loaded custom config", { path: Flag.CODY_CONFIG })
        }

        if (!Flag.CODY_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("cody", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CODY_CONFIG_DIR) {
          log.debug("loading config from CODY_CONFIG_DIR", { path: Flag.CODY_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.cody/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.CODY_CONFIG_CONTENT) {
          const source = "CODY_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODY_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from CODY_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODY_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODY_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CODY_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CODY_PERMISSION))
        }
        ...
        if (Flag.CODY_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CODY_DISABLE_PRUNE) {
