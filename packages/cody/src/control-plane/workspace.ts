...
import { Slug } from "@cody/core/util/slug"
...
export class Service extends Context.Service<Service, Interface>()("@cody/Workspace") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const startSync = Effect.fn("Workspace.startSync")(function* (space: Info) {
      if (!Flag.CODY_EXPERIMENTAL_WORKSPACES) return
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const env = {
        CODY_AUTH_CONTENT: JSON.stringify(yield* auth.all()),
        CODY_WORKSPACE_ID: config.id,
        CODY_EXPERIMENTAL_WORKSPACES: "true",
