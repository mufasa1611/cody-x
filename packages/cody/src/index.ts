  ...
  return new Promise((resolve) => {
    ...
    function render(first: boolean) {
      ...
      for (let i = 0; i < options.length; i++) {
        ...
        stdout.write(`  ${hl}${i === selected ? "❯" : " "} ${i + 1}. ${options[i].label}\x1B[K${rst}\n`)
      }
    ...
    const handler = (_str: string, key: { name?: string; ctrl?: boolean }) => {
      ...
      if (key.name === "1") { cleanup(); resolve(options[0].value); return }
      if (key.name === "2") { cleanup(); resolve(options[1].value); return }
    }
