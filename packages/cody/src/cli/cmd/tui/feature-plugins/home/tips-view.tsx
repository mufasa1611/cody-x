...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your Cody X conversation",
  ...
  "Create a project config file for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in your global config directory for all Cody X sessions",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Cody X auto-handles OAuth for remote MCP servers requiring auth",
  "Add custom command markdown files to define reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add agent markdown files for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Cody X auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Cody X uses LSP servers for intelligent code analysis",
  "Create local tool definition files to add new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add local plugin files for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Cody X from reading sensitive files",
  "Use {highlight}cody-x run{/highlight} for non-interactive scripting",
  "Use {highlight}cody-x --continue{/highlight} to resume the last session",
  "Use {highlight}cody-x run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}cody-x serve{/highlight} for headless API access to Cody X",
  "Use {highlight}cody-x run --attach{/highlight} to connect to a running server",
  "Run {highlight}cody-x upgrade{/highlight} to update to the latest version",
  "Run {highlight}cody-x auth list{/highlight} to see all configured providers",
  "Run {highlight}cody-x agent create{/highlight} for guided agent creation",
  "Use {highlight}/cody-x{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}cody-x github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/cody-x fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/cody-x{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in your Cody X themes directory",
  ...
  "Run {highlight}cody-x debug config{/highlight} to troubleshoot configuration",
  ...
  "Use the Cody X container image for containerized use when published",
  "Use {highlight}/connect{/highlight} with Cody X for curated, tested models",
