...
export class Service extends Context.Service<Service, Interface>()("@cody/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const projectFiles = Flag.CODY_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CODY_TUI_CONFIG override, if set.
  if (Flag.CODY_TUI_CONFIG) {
    const configFile = Flag.CODY_TUI_CONFIG
  ...
  for (const dir of dirs) {
    if (!dir.endsWith(".cody") && dir !== Flag.CODY_CONFIG_DIR) continue
