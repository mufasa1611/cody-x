...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run cody-x with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running cody-x server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CODY_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
