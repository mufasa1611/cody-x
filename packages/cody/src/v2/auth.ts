...
import { Identifier } from "@cody/core/util/identifier"
...
export class Service extends Context.Service<Service, Interface>()("@cody/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.CODY_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.CODY_AUTH_CONTENT)
        } catch {}
      }
