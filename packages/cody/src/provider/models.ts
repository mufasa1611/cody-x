...
export class Service extends Context.Service<Service, Interface>()("@cody/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.CODY_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.CODY_MODELS_PATH ?? filepath).pipe(
    ...
    if (!Flag.CODY_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
