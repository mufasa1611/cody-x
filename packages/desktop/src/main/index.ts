...
const TEST_ONBOARDING = process.env.CODY_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CODY_DISABLE_EMBEDDED_WEB_UI = "true"
  ...
  app.setAsDefaultProtocolClient("cody")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()
  ...
  const needsMigration = ((): boolean => {
    ...
    return !existsSync(join(base, "cody", "opencode.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.CODY_PORT
    if (fromEnv) {
