...
import { Select } from "@cody/ui/select"
import { Switch } from "@cody/ui/switch"
import { TextField } from "@cody/ui/text-field"
import { Tooltip } from "@cody/ui/tooltip"
import { useTheme, type ColorScheme } from "@cody/ui/theme/context"
...
export const SettingsGeneral: Component = () => {
  ...
  const check = () => {
    ...
    const doCheck = platform.checkUpdate
      ...
      : fetch(globalSdk.url + "/git-check", { method: "POST" }).then((r) =>
          r.json().then((body) => ({
        ...
        <SettingsRow
          ...
          description={
            <>
              ...
              <Link href="https://cody.ai/docs/themes/">{language.t("common.learnMore")}</Link>
            </>
          }
        >
          <Select
  ...
  return (
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      ...
      <div class="flex flex-col gap-8 w-full">
        ...
        <Show when={desktop() && import.meta.env.VITE_CODY_CHANNEL === "beta"}>
          <AdvancedSection />
        </Show>
      </div>
    </div>
