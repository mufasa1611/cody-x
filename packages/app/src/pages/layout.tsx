...
import { showToast, Toast, toaster } from "@cody/ui/toast"
...
import { retry } from "@cody/core/util/retry"
...
export default function Layout(props: ParentProps) {
    ...
    onMount(() => {
      ...
      const pollUpdate = () => {
        if (platform.checkUpdate) {
          return platform.checkUpdate().then(({ updateAvailable, version }) => {
        ...
        return fetch(globalSDK.url + "/global/git-check", { method: "POST" }).catch(() => {})
      }
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("https://cody.ai/desktop-feedback")}
      renderPanel={() =>
