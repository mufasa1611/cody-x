...
3. **Custom config**(`CODY_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `opencode.json`) - 프로젝트별 설정
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config**(`CODY_CONFIG_CONTENT` env var) - 런타임 override
...
`CODY_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`CODY_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.opencode` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
server/runtime config schema는 [**`opencode.ai/config.json`**](https://cody.dev/config.json)에 정의되어 있습니다.
...
`CODY_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.
...
`~/.config/opencode/agents/` 또는 `.cody/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
`~/.config/opencode/commands/` 또는 `.cody/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.cody/plugins/` 또는 `~/.config/opencode/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
