...
3. **Configuração personalizada** (`CODY_CONFIG` var de ambiente) - substituições personalizadas
...
6. **Configuração inline** (`CODY_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `CODY_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `CODY_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Use `CODY_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.cody/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.cody/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.cody/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
