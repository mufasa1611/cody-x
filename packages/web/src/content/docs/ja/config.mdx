...
3. **カスタム設定** (`CODY_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `opencode.json`) - プロジェクト固有の設定
5. **`.opencode` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`CODY_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド
...
`CODY_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`CODY_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
...
サーバー/ランタイム構成スキーマは [**`opencode.ai/config.json`**](https://cody.dev/config.json) で定義されています。
...
カスタム TUI 設定ファイルを指定するには、`CODY_TUI_CONFIG` を使用します。
...
`~/.config/opencode/agents/` または `.cody/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
`~/.config/opencode/commands/` または `.cody/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
プラグインファイルを `.cody/plugins/` または `~/.config/opencode/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
