...
Örneğin, genel ayarınız `theme: "cody"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
3. **Özel yapılandırma** (`CODY_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `opencode.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`CODY_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
`CODY_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`CODY_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
...
Yapılandırma dosyası [**`opencode.ai/config.json`**](https://cody.dev/config.json)'da tanımlanan bir şemaya sahiptir.
...
Agent'ları `~/.config/opencode/agents/` veya `.cody/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/opencode/commands/` veya `.cody/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.cody/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
