...
Por ejemplo, si su configuración global establece `theme: "cody"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
3. **Configuración personalizada** (`CODY_CONFIG` env var): anulaciones personalizadas
...
6. **Configuración en línea** (`CODY_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `CODY_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `CODY_CONFIG_DIR`
...
El archivo de configuración tiene un esquema definido en [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.cody/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.cody/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.cody/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
