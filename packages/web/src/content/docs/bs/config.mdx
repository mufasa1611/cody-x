...
3. **Prilagođena konfiguracija** (`CODY_CONFIG` env var) - prilagođena preinačenja
...
6. **Inline konfiguracija** (`CODY_CONFIG_CONTENT` env var) - runtime preinačenja
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODY_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODY_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Koristite `CODY_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.cody/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.cody/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.cody/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
