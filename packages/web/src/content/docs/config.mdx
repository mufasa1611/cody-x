...
3. **Custom config** (`CODY_CONFIG` env var) - custom overrides
4. **Project config** (`opencode.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`CODY_CONFIG_CONTENT` env var) - runtime overrides
...
Specify a custom config file path using the `CODY_CONFIG` environment variable.
...
Specify a custom config directory using the `CODY_CONFIG_DIR`
...
The server/runtime config schema is defined in [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Use `CODY_TUI_CONFIG` to point to a custom TUI config file.
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.cody/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.cody/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.cody/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.
