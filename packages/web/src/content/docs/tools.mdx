...
:::note
This tool is only available when `CODY_EXPERIMENTAL_LSP_TOOL=true` (or `CODY_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `CODY_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
