...
3. **Configuration personnalisée** (`CODY_CONFIG` env var) - remplacements personnalisés
...
6. **Configuration en ligne** (`CODY_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `CODY_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `CODY_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.
...
Le fichier de configuration a un schéma défini dans [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Utilisez `CODY_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.cody/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.cody/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.cody/plugins/` ou `~/.config/opencode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
