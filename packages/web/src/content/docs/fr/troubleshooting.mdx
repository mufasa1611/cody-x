...
Si `CODY_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `CODY_PORT` (ou choisissez un port libre) et redémarrez.
