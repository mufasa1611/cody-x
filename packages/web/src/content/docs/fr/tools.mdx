...
:::note
Cet outil n'est disponible que lorsque `CODY_EXPERIMENTAL_LSP_TOOL=true` (ou `CODY_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `CODY_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
