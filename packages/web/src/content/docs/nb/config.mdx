...
For eksempel, hvis din globale konfigurasjon setter `theme: "cody"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
3. **Egendefinert konfigurasjon** (`CODY_CONFIG` env var) - egendefinerte overstyringer
...
6. **Inline config** (`CODY_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `CODY_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `CODY_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.cody/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.cody/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.cody/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
