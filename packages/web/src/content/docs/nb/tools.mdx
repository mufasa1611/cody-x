...
:::note
Dette verktøyet er bare tilgjengelig når `CODY_EXPERIMENTAL_LSP_TOOL=true` (eller `CODY_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `CODY_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
