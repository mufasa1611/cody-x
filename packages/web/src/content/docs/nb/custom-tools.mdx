...
- Lokalt ved å plassere dem i `.cody/tools/`-katalogen til prosjektet ditt.
...
```ts title=".cody/tools/database.ts" {1}
...
```ts title=".cody/tools/project.ts" {8}
import { tool } from "@cody/plugin"
...
```python title=".cody/tools/add.py"
import sys
...
```ts title=".cody/tools/python-add.ts" {10}
import { tool } from "@cody/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".cody/tools/add.py")
