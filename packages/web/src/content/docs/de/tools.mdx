...
:::note
Dieses Tool ist nur verfuegbar, wenn `CODY_EXPERIMENTAL_LSP_TOOL=true` (oder `CODY_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `CODY_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
