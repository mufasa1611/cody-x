...
Wenn Ihre globale Konfiguration beispielsweise `theme: "cody"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
3. **Benutzerdefinierte Konfiguration** (`CODY_CONFIG` env var) – benutzerdefinierte Überschreibungen
...
6. **Inline-Konfiguration** (`CODY_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
Geben Sie mithilfe der Umgebungsvariablen `CODY_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `CODY_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`opencode.ai/config.json`**](https://cody.dev/config.json) definiert ist.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.cody/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.cody/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.cody/plugins/` oder `~/.config/opencode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
