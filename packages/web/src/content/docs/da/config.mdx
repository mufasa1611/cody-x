...
3. **Tilpasset konfiguration** (`CODY_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`opencode.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`CODY_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `CODY_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `CODY_CONFIG_DIR`
...
Konfigurationsfilen har et skema, der defineres i [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.cody/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.cody/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.cody/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
