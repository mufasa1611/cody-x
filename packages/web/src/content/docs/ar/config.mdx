...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "cody"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
3. **إعدادات مخصصة** (`CODY_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`opencode.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.opencode`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`CODY_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `CODY_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `CODY_CONFIG_DIR`.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/opencode/agents/` أو `.cody/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/opencode/commands/` أو `.cody/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.cody/plugins/` أو `~/.config/opencode/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
