...
:::note
هذه الأداة متاحة فقط عند ضبط `CODY_EXPERIMENTAL_LSP_TOOL=true` (أو `CODY_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `CODY_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
