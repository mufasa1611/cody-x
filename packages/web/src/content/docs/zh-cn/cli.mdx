...
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本认证密码（默认使用 `CODY_SERVER_PASSWORD`）                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `CODY_SERVER_USERNAME` 或 `opencode`） |
...
此命令启动一个 HTTP 服务器，提供对 OpenCode 功能的 API 访问，无需 TUI 界面。设置 `CODY_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenCode。设置 `CODY_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
| 变量                                  | 类型    | 描述                                    |
| ------------------------------------- | ------- | --------------------------------------- |
| `CODY_AUTO_SHARE`                 | boolean | 自动分享会话                            |
| `CODY_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可执行文件的路径    |
| `CODY_CONFIG`                     | string  | 配置文件路径                            |
| `CODY_TUI_CONFIG`                 | string  | TUI 配置文件路径                        |
| `CODY_CONFIG_DIR`                 | string  | 配置目录路径                            |
| `CODY_CONFIG_CONTENT`             | string  | 内联 JSON 配置内容                      |
| `CODY_DISABLE_AUTOUPDATE`         | boolean | 禁用自动更新检查                        |
| `CODY_DISABLE_PRUNE`              | boolean | 禁用旧数据清理                          |
| `CODY_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自动终端标题更新                    |
| `CODY_PERMISSION`                 | string  | 内联 JSON 权限配置                      |
| `CODY_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用默认插件                            |
| `CODY_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用 LSP 服务器自动下载                 |
| `CODY_ENABLE_EXPERIMENTAL_MODELS` | boolean | 启用实验性模型                          |
| `CODY_DISABLE_AUTOCOMPACT`        | boolean | 禁用自动上下文压缩                      |
| `CODY_DISABLE_CLAUDE_CODE`        | boolean | 禁用读取 `.claude`（提示词 + 技能）     |
| `CODY_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 禁用读取 `~/.claude/CLAUDE.md`          |
| `CODY_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用加载 `.claude/skills`               |
| `CODY_DISABLE_MODELS_FETCH`       | boolean | 禁用从远程源获取模型                    |
| `CODY_FAKE_VCS`                   | string  | 用于测试目的的模拟 VCS 提供商           |
| `CODY_CLIENT`                     | string  | 客户端标识符（默认为 `cli`）            |
| `CODY_ENABLE_EXA`                 | boolean | 启用 Exa 网络搜索工具                   |
| `CODY_SERVER_PASSWORD`            | string  | 为 `serve`/`web` 启用基本认证           |
| `CODY_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `opencode`） |
| `CODY_MODELS_URL`                 | string  | 自定义模型配置获取 URL                  |
...
| 变量                                            | 类型    | 描述                            |
| ----------------------------------------------- | ------- | ------------------------------- | --- | -------------------------------- | ------- | ------------------------ |
| `CODY_EXPERIMENTAL`                         | boolean | 启用所有实验性功能              |
| `CODY_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 启用图标发现                    |
| `CODY_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 禁用 TUI 中的选中即复制         |
| `CODY_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 命令的默认超时时间（毫秒） |
| `CODY_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 响应的最大输出 Token 数     |
| `CODY_EXPERIMENTAL_FILEWATCHER`             | boolean | 启用整个目录的文件监听器        |
| `CODY_EXPERIMENTAL_OXFMT`                   | boolean | 启用 oxfmt 格式化器             |
| `CODY_EXPERIMENTAL_LSP_TOOL`                | boolean | 启用实验性 LSP 工具             |
| `CODY_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用文件监听器                  |
| `CODY_EXPERIMENTAL_EXA`                     | boolean | 启用实验性 Exa 功能             |
| `CODY_EXPERIMENTAL_LSP_TY`                  | boolean | 为 python 文件启用 TY LSP       |     | `CODY_EXPERIMENTAL_MARKDOWN` | boolean | 启用实验性 Markdown 功能 |
| `CODY_EXPERIMENTAL_PLAN_MODE`               | boolean | 启用计划模式                    |
