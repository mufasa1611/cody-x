...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "cody"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
3. **Konfiguracja niestandardowa** (zmienna `CODY_CONFIG`) — jawne nadpisanie
...
6. **Konfiguracja wbudowana** (zmienna `CODY_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CODY_CONFIG`.

```bash
export CODY_CONFIG=/path/to/my/custom-config.json
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CODY_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.

```bash
export CODY_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Użyj `CODY_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencode/agents/` lub `.cody/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.cody/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.cody/plugins/` lub `~/.config/opencode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
