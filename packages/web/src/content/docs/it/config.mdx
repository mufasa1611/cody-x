...
3. **Config personalizzata** (variabile d'ambiente `CODY_CONFIG`) - sovrascritture personalizzate
...
6. **Config inline** (variabile d'ambiente `CODY_CONFIG_CONTENT`) - sovrascritture a runtime
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CODY_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CODY_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`opencode.ai/config.json`**](https://cody.dev/config.json).
...
Usa `CODY_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.cody/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.cody/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.cody/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
