...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `CODY_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `CODY_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `CODY_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| `CODY_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
