...
const CHANNEL = await (async () => {
  if (env.CODY_CHANNEL) return env.CODY_CHANNEL
  if (env.CODY_BUMP) return "latest"
  if (env.CODY_VERSION && !env.CODY_VERSION.startsWith("0.0.0-")) return "latest"
...
const bot = ["actions-user", "cody", "cody-agent[bot]"]
...
export const Script = {
  ...
  get release(): boolean {
    return !!env.CODY_RELEASE
  },
  get team() {
    return team
  },
}
console.log(`cody script`, JSON.stringify(Script, null, 2))
