...
We want to make it easy for you to contribute to Cody. Here are the most common type of changes that get merged:
...
By default, `bun dev` runs Cody in the `packages/cody` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Cody in the root of the cody repo itself:
...
```bash
./packages/cody/script/build.ts --single
```

Then run it with:

```bash
./packages/cody/dist/cody-<platform>/bin/cody
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/cody`: Cody core business logic & server.
  - `packages/cody/src/cli/cmd/tui/`: The TUI code, written in SolidJS with [opentui](https://github.com/sst/opentui)
...
To start the Cody headless API server:
...
1. **First, start the Cody server** (see [Running the API Server](#running-the-api-server) section above)
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/cody/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
The most reliable way to debug Cody is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
your debugger via that URL. Other methods can result in breakpoints being mapped incorrectly, at least in VSCode (YMMV).

Caveats:

- If you want to run the Cody TUI and have breakpoints triggered in the server code, you might need to run `bun dev spawn` instead of
  the usual `bun dev`. This is because `bun dev` runs the server in a worker thread and breakpoints might not work there.
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/cody ./src/index.ts serve --port 4096`,
    then attach TUI with `cody attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/cody --conditions=browser ./src/index.ts`
...
- The same problem arises when running Cody in the VSCode `JavaScript Debug Terminal`
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in Cody. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
...
- **Denounced users** are explicitly blocked. Issues and pull requests from denounced users are automatically closed. If you have been denounced, you can request to be unvouched by reaching out to a maintainer on [Discord](https://cody.ai/discord)
